...
public interface TweetRepository extends JpaRepository<TweetEntity, Long> {
    ...
    @Query(value = "Select * FROM tweet_entity WHERE hash_tags like %:hashText% order by created_dt desc", nativeQuery = true)
    ...
    @Query(value = "Select * FROM tweet_entity WHERE userName like %:userName%  AND hash_tags like %:hashText% order by created_dt desc", nativeQuery = true)
	List<TweetEntity> getAllTweetsByNameAndHashTag(@Param("userName") String userName, @Param("hashText") String tweetText);

    @Query(value = "Select * FROM tweet_entity WHERE userName like %:userName%  AND hash_tags like %:hashText% AND created_dt >=:fromDate and created_dt <=:toDate order by created_dt desc", nativeQuery = true)
    ...
    @Query(value = "Select * FROM tweet_entity WHERE hash_tags like %:hashText% AND created_dt >= :fromDate and created_dt <=:toDate order by created_dt desc", nativeQuery = true)
